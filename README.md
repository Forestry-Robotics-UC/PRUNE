# <img src="docs/assets/fruc_logo.png" width="150" alt="FRUC logo"/>  PRUNE — Projection Rejection of Unreliable Neighborhood Evidence
...
and temporal fusion live elsewhere. PRUNE is training-free, requires no neural
inference, and is model-agnostic with respect to the upstream perception model.

**Demo:** PRUNE functionality

[▶ Watch the demo (mp4)](docs/assets/prune_functions_live_demo.mp4)

**Segmented Output:**

![Example Segmented Projection](docs/assets/example_sensor_fusion.gif)


## Citation

If you use PRUNE in your research, please cite:

```bibtex
@inproceedings{
andrada2026prune,
title={{PRUNE}: Selective Li{DAR}{\textendash}Camera Projection via Multi-Stage Evidence Rejection for Semantic Fusion in Unstructured Forests},
author={Andrada, M. E., Portugal, David, Peixoto, Paulo, Kantor, George},
booktitle={The 16th International Conference on Information Science and Technology},
year={2026},
url={https://openreview.net/forum?id=dUU8Bm6LCG}
}
```
...
- `prune_ros/`: catkin ROS wrapper package.
  ...
  - `prune_ros/node/prune_node.py`: node implementation;
...
- To decompress compressed topics, set `use_republish:=true` and provide base input topics (no `/compressed` suffix), e.g.:
  ```bash
  roslaunch prune_ros prune.launch use_republish:=true semantic_in_topic:=/segmentation/test
  ```
