...
from entfac_fusion_ros.conversions import pointcloud2_to_xyz, pointcloud2_to_xyz_t
...
class LidarFusionPipeline:
            ...
            if lidar_frame:
                ...
                if camera_T_lidar is None:
                    self._node._log.debug(
                        ...
                        lidar_frame,
                        self._node.camera_frame,
                    )
                    camera_T_lidar = self._node._lookup_transform(
                        self._node.camera_frame, lidar_frame, rospy.Time(0)
                    )
                ...
                if target_T_lidar is None:
                    ...
                    target_T_lidar = self._node._lookup_transform(
                        self._node.target_frame, lidar_frame, rospy.Time(0)
                    )
    ...
    def _lookup_imu_omega(self, stamp: rospy.Time) -> Optional[np.ndarray]:
        ...
        if self._node.imu_cache_max_dt_sec > 0.0 and best_dt > self._node.imu_cache_max_dt_sec:
            return None
        imu_frame = self._node.imu_frame or (
            after.header.frame_id if before is None else before.header.frame_id
        )
        if not imu_frame:
            return None
    ...
    def _lookup_lidar_imu(self, stamp: rospy.Time) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        ...
        if self._node.lidar_imu_cache_max_dt_sec > 0.0 and best_dt > self._node.lidar_imu_cache_max_dt_sec:
            return None
        imu_frame = self._node.lidar_imu_frame or (
            before.header.frame_id if before is not None else after.header.frame_id
        )
        if not imu_frame:
            return None
    ...
    def _deskew_lidar_points(
        ...
        if t_raw is None or t_raw.size == 0:
            self._node._lidar_deskew_status = (
                f"armed (missing point time field '{self._node.lidar_time_field}')"
            )
            now = time.time()
        ...
        if now - self._node._lidar_deskew_log_at > 2.0:
            self._node._log.info(
                ...
                self._node.lidar_deskew_mode,
                self._node.lidar_deskew_ref,
                scan_span,
                sample_count,
            )
    ...
    def _process_frame(
        ...
        *,
        ...
        if self._node._calibration is not None and semantic_debug_img is not None:
            ...
            corrected_camera_T_lidar, calib_snapshot = self._node._calibration_bridge.update(
                points=points,
        ...
        rolling_shutter_readout_sec = (
            self._get_readout_sec(sem_msg.header.stamp) if self._node.rolling_shutter_enable else 0.0
        )
        rolling_shutter_omega_cam = (
            self._lookup_imu_omega(sem_msg.header.stamp) if rolling_shutter_readout_sec > 0.0 else None
        )
        proj_result = self._node._projector.process_frame(
            points=points,
            labels=labels,
        ...
        projection_metrics = proj_result.metrics
        ...
        if self._node._debug_pub is not None:
            if self._node.debug_project_lidar:
                ...
                uv, _ = project_points_to_image(
                    points, intrinsics, corrected_camera_T_lidar, (image_shape[1], image_shape[0])
                )
                self._node._diagnostics.publish_lidar_projection(
                    base_rgb, image_shape, uv, sem_msg.header, colors_u8=debug_colors
                )
            if self._node.debug_publish_fov_points and points.shape[0]:
                self._node._diagnostics.publish_fov_points(
        ...
        return _LidarFrameResult(
            ...
            depth_map=proj_result.depth_map,
            edge_map=proj_result.edge_map,
    ...
    def _assemble_and_publish(
        ...
        *,
        ...
        if (
            self._node._debug_pub is not None
            and self._node.debug_range_view
            and semantic_debug_img is not None
            and pcl.points_xyz.shape[0]
        ):
            dmap = depth_map if depth_map is not None else self._node._projector._rasterize_depth_map(
                points, intrinsics, corrected_camera_T_lidar, (h, w)
            )
            emap = edge_map if edge_map is not None else self._node._projector._depth_to_edge_map(dmap)
            self._node._diagnostics.publish_range_view(
                depth_map=dmap,
                edge_map=emap,
                sem_img=semantic_debug_img,
                sem_type=semantic_debug_type,
        ...
        if self._node._tracked_repr is not None and semantic_debug_img is not None:
            ...
            emap = edge_map if edge_map is not None else self._node._projector._depth_to_edge_map(dmap)
            tracked = self._node._tracked_runtime.update(
                sem_img=semantic_debug_img,
                sem_type=semantic_debug_type,
                depth_map=dmap,
                depth_edges=emap,
            ...
            if tracked is not None:
                self._node._diagnostics.publish_tracked_reprojection(
                    tracked.overlay_img, tracked.error_px, sem_msg.header
                )
    ...
    def process(self, sem_msg, lidar_msg, conf_msg=None, invalid_mask_msg=None):
        ...
        lidar_result = self._process_frame(
            sem_msg=sem_msg,
            lidar_msg=lidar_msg,
            labels=labels,
            packed_img=packed_img,
