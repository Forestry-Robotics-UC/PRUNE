...
Required:
  ...
  - Geometry input:
    ...
    - Deprecated: ``~depth_topic`` and ``~lidar_topic``.
...
class SemanticPclNode:
    ...
    def _register_subscribers(self):
        ...
        if self.mode == "depth":
            if not self.depth_topic:
                raise ValueError("Depth mode requires ~depth_topic or ~depth_input_topic")
            depth_sub = Subscriber(self.depth_topic, Image)
            subs = [semantic_sub, depth_sub]
            if conf_sub is not None:
                subs.append(conf_sub)
            sync = ApproximateTimeSynchronizer(subs, queue_size=5, slop=0.1)
            sync.registerCallback(self._depth_callback)
            self._sync = sync
        else:
            if not self.lidar_topic:
                raise ValueError("LiDAR mode requires ~lidar_topic or ~depth_input_topic")
    ...
    def _detect_mode(self):
        ...
        if depth_set and not lidar_set:
            self._mode_source = "forced"
            self._mode_detail = "forced via ~depth_topic"
            return "depth"
        if lidar_set and not depth_set:
            self._mode_source = "forced"
            self._mode_detail = "forced via ~lidar_topic"
            return "lidar"
        if not depth_set and not lidar_set:
            raise ValueError("Set ~depth_input_topic or ~depth_topic/~lidar_topic")

        if (
            self._wait_for_msg(self.depth_topic, Image, timeout=0.2, warn_on_timeout=False)
            is not None
        ):
            self._mode_source = "auto"
            self._mode_detail = "auto via first message on ~depth_topic"
            return "depth"
        if (
            self._wait_for_msg(
            ...
            self._mode_source = "auto"
            self._mode_detail = "auto via first message on ~lidar_topic"
            return "lidar"
        ...
        self._mode_source = "auto"
        self._mode_detail = "auto fallback (no messages seen) -> depth"
        return "depth"
    ...
    def _prime_transforms(self):
        if self.mode == "depth":
            if self.static_target_T_depth is not None:
                ...
                return
            if self.depth_topic:
                msg = self._wait_for_msg(self.depth_topic, Image, timeout=5.0)
                if msg is None:
                    self._log.debug(
                        "_prime_transforms",
        ...
        lidar_msg = None
        if self.lidar_topic:
            lidar_msg = self._wait_for_msg(self.lidar_topic, PointCloud2, timeout=5.0)
    ...
    def _srv_save_ply(self, req: Trigger.Request) -> TriggerResponse:
        ...
        return TriggerResponse(success=False, message="writer queue full")
    ...
    def _render_startup_table(self) -> str:
        ...
        ply = f"recording={self._ply_recording} dir={self.ply_output_dir}"
        help_text = "\n".join(
            [
        ...
        rows = [
            ...
            ("depth_input", self.depth_input_topic or "-"),
            ("depth_topic", self.depth_topic or "-"),
            ("lidar_topic", self.lidar_topic or "-"),
