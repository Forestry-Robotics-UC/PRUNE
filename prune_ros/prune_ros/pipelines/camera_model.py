...
from prune_ros.config import load_camera_info_txt as _load_camera_info_txt_helper
...
class CameraModel:
    ...
    def load(self) -> CameraInfoData:
        if self._node.camera_info_txt:
            ...
            self._node._camera_info_source = f"txt:{resolved_path}"
            self._log.info(
                "__init__",
            ...
            self._log.info(
                ...
                int(self._node._camera_info_size[1]),
                int(self._node._camera_info_size[0]),
            )
        else:
